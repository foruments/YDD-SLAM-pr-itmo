...
- Протестировать систему на собственных данных (iPhone 16 Pro LiDAR)
- Оценить точность в динамических сценах

**Команда:** [Твое имя] (индивидуальная работа)
...
- **Запись собственных данных:** Использование iPhone 16 Pro + Stray Scanner (3676 кадров)
- **Конвертация данных:** Разработка `iphone_converter.py` для преобразования в TUM формат
- **Оценка результатов:** Расчет метрик ATE через `evaluate_ate.py`, построение графиков
- **Анализ:** Сравнение с оригинальной статьей, выявление причин дрейфа на iPhone данных
...
**Текущие ограничения:**
- Отсутствие loop closing → дрейф на длинных траекториях
- Чувствительность к низкотекстурным сценам (белые стены)
- Шум LiDAR сенсора iPhone

**Возможные улучшения:**
- Интеграция алгоритма закрытия петель (loop closure)
- Использование IMU данных для уменьшения дрейфа
- Тестирование на уличных сценах с богатой текстурой
